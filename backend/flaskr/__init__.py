...
def create_app(test_config=None):
    ...
    def get_questions_per_categories(categorie_id):
        try:
            
            categorie = Category.query.filter_by(id = categorie_id).first()

            if categorie:

                questions = Question.query.filter(Question.category == categorie_id).all()
                question_format = [question.format() for question in questions]
                ...
                return jsonify(
                    {
                        ...
                        "questions": question_format,
                        "total_questions" : len(question_format)
                    }
                 )
            else:
                abort(404)
    ...
    def get_quizzes():
        ...
        try:   
            ...
            for question in questions:
                if not question.id in array:
                    data.append(question)
            question_random = random.choice(data)
            formatted_question = question_random.format()
            return jsonify({
